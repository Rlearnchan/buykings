...
이 문서는 위폴 CSV 원본과 현재까지 산출한 지수를 SQLite에 적재하는 운영 기준을 정리한다.

핵심 원칙은 간단하다.

- 스레드에는 계속 CSV를 던진다.
- 원본 CSV는 사람이 보기 쉬운 파일 형태로 유지한다.
- 뒤에서는 SQLite를 정본 저장소처럼 사용한다.
...
현재 적재 스크립트는 표준 라이브러리 `sqlite3`를 사용하므로 추가 설치가 필요 없다.
...
3. 결과를 검토한다.
4. 적재 스크립트로 raw/state를 SQLite에 반영한다.
