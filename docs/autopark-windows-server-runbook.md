...
`../syuka-ops`처럼 컨테이너에서 실행 환경과 반복 루프를 관리할 수도 있다. Autopark는 X 로그인 세션이
필요하므로, 1차 Docker 구조는 컨테이너 내부 runner가 호스트 Chrome CDP에 붙는 방식이다.

호스트에서 Chrome CDP 프로필을 먼저 띄운다.
...
기본값은 `AUTOPARK_SKIP_PUBLISH=1`, `AUTOPARK_SKIP_DATAWRAPPER_EXPORT=1`이다. 발행까지 테스트하려면
환경변수로 명시한다.

```powershell
$env:AUTOPARK_SKIP_PUBLISH="0"
