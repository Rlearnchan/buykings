...
마감 직전에는 차트 안의 설명 요소도 같이 확인한다.

- `annotate.notes`가 draft 문구가 아니라 최종 분석 기간 문구인지
- 시계열 highlight `range-annotations`가 실제 이번 주 구간과 맞는지
- 버블 `week_range`, day label, annotate 문구가 같은 주간을 가리키는지

예:

- 시계열 주석: `분석 기간: 2026.03.09 ~ 2026.04.19, 지수는 50(점선)을 기준으로 해석`
- 버블 주석: `분석 기간: 2026.04.13 ~ 2026.04.19, 순서대로 월(1)~일(7)`
- 시계열 highlight: `2026-04-13 ~ 2026-04-19`
- `post` -> `게시물 수`
- `경계` -> `신중`
