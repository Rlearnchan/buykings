...
|---|---|---|---|---|
| Preflight Agenda | 조간 편집회의 전 뉴스 에디터 | `gpt-5-mini` | 밤사이 주요 의제와 확인 질문을 제안 | 최종 기사 배치 확정 |
| Evidence Microcopy | 자료 정리 인턴 또는 리서처 | `gpt-5-mini` | 자료 하나하나의 핵심을 짧게 요약 | 자료 채택/순위 결정 |
| Market Focus | 증권부 데스크 | `gpt-5-mini` | 시장 근거, 가격 반응, source gap 검증 | publish 형식 변경 |
| Editorial Brief | 편집국장 | `gpt-5-mini` | 톱스토리, 방송 흐름, 한국장 연결점 결정 | 카드명/번호 임의 변경 |
