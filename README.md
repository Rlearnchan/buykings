...
1. Docker Compose 스케줄러가 날짜별 job을 실행한다.
...
Codex에 걸려 있던 운영 자동화 세 개는 이제 Docker Compose 서비스가 맡는다.

- Autopark 아침 대시보드
  - compose file: `docker-compose.autopark.yml`
  - services: `autopark-browser`, `autopark-publisher`
  - schedule: `AUTOPARK_RUN_TIMES=06:00`, `AUTOPARK_RETRY_TIMES=06:20` KST
  - behavior: containerized Chromium CDP가 healthy가 된 뒤 `projects/autopark/scripts/run_live_dashboard_all_in_one.py`를 실행하고, `AUTOPARK_PUBLISH_POLICY=gate` 기준으로 Notion/Datawrapper 발행을 진행한다.
- Autopark 방송 후 회고
  - compose file: `docker-compose.autopark.yml`
  - service: `autopark-retrospective`
  - schedule: `AUTOPARK_RETRO_RUN_TIMES=10:30` KST
  - behavior: `projects/autopark/scripts/run_broadcast_retrospective.py`를 `AUTOPARK_RETRO_ATTEMPTS=6`, `AUTOPARK_RETRO_SLEEP_MINUTES=60` 기준으로 실행한다.
- 위폴은 지금 일일 업데이트
  - compose file: `docker-compose.wepoll.yml`
  - services: `wepoll-daily-scheduler`, optional `wepoll-fetcher`
  - schedule: `WEPOLL_RUN_TIMES=00:10`, `WEPOLL_RETRY_TIMES=00:30` KST
  - behavior: KST 기준 어제 날짜를 계산해 `scripts/run_wepoll_daily_from_fetcher.py`를 실행하고, 기존 Datawrapper chart id `jRh1f`, `Dd29j`를 유지한다. 현재 Windows 운영에서는 로그인된 headed fetcher를 `http://127.0.0.1:8777`에 띄우고 Docker daily runner가 `http://host.docker.internal:8777`로 붙는 hybrid 구성이 기본이다.
...
Invoke-RestMethod -Uri http://127.0.0.1:8777/health
```

서비스를 다시 올릴 때는 아래처럼 실행한다.
...
- [docs/wepoll-docker.md](docs/wepoll-docker.md)
  - 위폴 daily fetcher/runner/scheduler Docker 운영 방식
