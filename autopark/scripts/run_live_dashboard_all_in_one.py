...
RUNTIME_COPY_PATTERNS = [
    ("notion", "autopark/runtime/notion/{date}"),
    ("processed", "autopark/data/processed/{date}"),
    ("reviews", "autopark/runtime/reviews/{date}"),
    ("logs", "autopark/runtime/logs/{date}*"),
    ("charts", "autopark/charts/*datawrapper.json"),
    ("prepared", "autopark/prepared/*{date}*"),
    ("exports-current", "autopark/exports/current/*"),
]
...
def launch_chrome_cdp(args: argparse.Namespace, env: dict[str, str], results: list["StepResult"]) -> StepResult:
    ...
    if IS_MAC:
        ...
        result, _ = run(["bash", "autopark/scripts/launch_chrome_cdp_profile.sh"], "launch chrome cdp profile", 90)
        return result
...
def main() -> int:
    ...
    result, _ = run(
        [
            py,
            "autopark/scripts/preflight_0430.py",
            "--date",
            args.date,
            "--cdp-endpoint",
            args.cdp_endpoint,
    ...
    result, _ = run(
        [
            ...
            args.date,
            "--input",
            f"autopark/data/processed/{args.date}/earnings-calendar-x-posts.json",
        ],
    ...
    result, _ = run([py, "autopark/scripts/build_visual_cards.py", "--date", args.date], "build visual cards", 120, allow_fail=True)
    append_step(results, result)

    if not args.skip_finviz:
        if IS_MAC:
            result, _ = run(
                ["bash", "autopark/scripts/sync_chrome_profile_for_playwright.sh"],
        ...
        for source in FINVIZ_SOURCES:
            result, _ = run(
                [
                    node,
                    "autopark/scripts/capture_source.mjs",
                    "--date",
                    args.date,
                    "--source",
                    source,
    ...
    for chart in MARKET_CHARTS:
        result, _ = run(
            [
                py,
                "autopark/scripts/fetch_market_chart_data.py",
                "--date",
                args.date,
                "--chart",
                chart,
    ...
    result, _ = run(
        [
            py,
            "autopark/scripts/fetch_economic_calendar.py",
            "--date",
            args.date,
            "--min-importance",
            "2",
    ...
    result, _ = run(
        [py, "autopark/scripts/build_earnings_ticker_drilldown.py", "--date", args.date],
    ...
    result, dashboard_payload = run([py, "autopark/scripts/build_live_notion_dashboard.py", "--date", args.date], "build notion markdown", 90)
    append_step(results, result)
    ...
    result, review_payload = run([py, "autopark/scripts/review_dashboard_quality.py", "--date", args.date, "--json"], "review dashboard quality", 90, allow_fail=True)
    append_step(results, result)
    ...
    if should_publish:
        result, publish_payload = run(
            [py, "autopark/scripts/publish_recon_to_notion.py", "--replace-existing", dashboard_path],
            "publish notion",
            240,
        )
        append_step(results, result)
