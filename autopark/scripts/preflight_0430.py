...
def check_cdp(endpoint: str) -> Check:
    ...
    try:
        ...
        elapsed = time.monotonic() - started
        details = f"{endpoint} unavailable: {exc}. Run ops/windows/start_autopark_chrome.ps1 or autopark/scripts/launch_chrome_cdp_profile.sh first."
...
def render_markdown(date_value: str, checks: list[Check]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Morning Notes",
            "",
            "- X가 fail이면 Windows에서는 `ops/windows/start_autopark_chrome.ps1`, Mac에서는 `autopark/scripts/launch_chrome_cdp_profile.sh`를 먼저 실행하고 재시도한다.",
